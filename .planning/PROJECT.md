...
- [ ] User can view the UR3e cell in an interactive 3D scene
- [ ] User can orbit/pan/zoom the 3D camera, and reset the camera to center on the robot cell
- [ ] User sees a Fusion-360-style navigation cube (Front/Top/Bottom/Back views) in the 3D viewport that rotates in sync with the camera
...
| Web app (browser-based) platform | User chose web app over desktop app | — Pending |
| Prioritize 3D toolpath simulation as the must-nail centerpiece | Tight few-day timeline; user named this as the one thing that must impress | — Pending |
