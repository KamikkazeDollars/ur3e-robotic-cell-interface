...
- [x] 01-03-PLAN.md — shadcn/ui design-system layer, UI-SPEC token contract, and the one-action camera reset (SCENE-02)
...
- [x] 02-02-PLAN.md — G2/G3 arc tessellation, the milling sample with depth passes, input guards, and the committed sample/anchor test suite (SIM-01, SIM-02)
...
- [x] 03-01-PLAN.md — Tracer: drag the scrub control and watch the UR3e re-pose along the toolpath via closed-form IK, pinned by round-trip, continuity and rail-resolution tests (SIM-05)
...
- [x] 03-04-PLAN.md — G-03-6: one token-backed scene palette replacing three copied tone constants, giving the rail rig, workbench and floor three distinct colors with an anti-drift test (SCENE-03, SCENE-04)

**Gap closure — Wave 2** *(blocked on 03-04)*

- [x] 03-05-PLAN.md — G-03-4: toolpath marker radius derived from `ParsedToolpath.bounds` instead of a hardcoded constant, plus vertical guide stems and tabletop footprints under each marker; marker COUNT stays deferred to Phase 6 (SIM-02, SIM-05)
...
- [x] 04-04-PLAN.md — Traversed-path highlight over the toolpath, larger gated scrub marker, playback clock moved ahead of every per-frame consumer
- [x] 04-05-PLAN.md — Mode-tagged bundled samples, picker filtered by the active cell mode, re-selection on mode change
